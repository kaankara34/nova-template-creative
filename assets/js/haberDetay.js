const newsData = [
  {
    ...
    photos1: [
      ...
      "assets/images/news/atasehir/atasehir-imza2.jpg",
      "assets/images/news/atasehir/atasehir-imza3.jpg",
      "assets/images/news/atasehir/atasehir-imza4.jpg",
    ],
  ...
  },
  {
    ...
    date: "27 Kasım 2023",
    paragraphs: [
      "Ataşehir’in Barbaros Mahallesi’nde yer alan 40T adası için önemli bir adım atıldı. Kadıköy 35. Noter'de gerçekleştirilen imza töreni, yoğun bir katılımla gerçekleşti. Proje kapsamında atılan bu adım, bölgedeki gayrimenkul projelerine yönelik heyecanı artırırken, yatırımcılar ve yerel halk tarafından büyük ilgiyle karşılandı.",
    ],
    photos1: [
      "assets/images/news/atasehir/atasehir-imza.jpg",
      "assets/images/news/osmankara/osm-imza.jpg",
    ],
    postPhoto: "assets/images/news/atasehir/atasehir-imza.jpg",
    blockquote: {
      text: "Proje kapsamında atılan bu adım, bölgedeki gayrimenkul projelerine yönelik heyecanı artırırken, yatırımcılar ve yerel halk tarafından büyük ilgiyle karşılandı.",
    },
  },
  {
    id: 3,
  ...
  },
  {
    ...
    paragraphs2: [
      "Hem yenilenebilir enerji hem de akıllı ev teknolojileri konularında somut adımlar atılması için taraflar bu alanlarda ortak projeler geliştirme konusunda istekli olduklarını belirtti. Özellikle yenilenebilir enerji projeleri, gelecekte iki ülke arasında ekonomik ve teknolojik bağları güçlendirecek önemli bir işbirliği alanı olarak gözüküyor.",
    ],
    photos1: [
      "assets/images/news/osmankara/malezya1.jpg",
      "assets/images/news/osmankara/malezya2.jpg",
  ...
  },
  {
    ...
    photos1: [
      ...
      "assets/images/news/osmankara/rachmad_odul2.jpg",

      "assets/images/news/osmankara/rachmad_odul4.jpg",
      "assets/images/news/osmankara/rachmad_odul5.jpg",
    ],
...
// Dinamik içerik ekleme işlemleri
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Haber verisini id'ye göre bul
  const newsItem = newsData.find((item) => item.id === newsId);

  if (newsItem) {
    document.title = `NOVA © - ${newsItem.title}`;
    document.getElementById("post_photo").src = newsItem.postPhoto;
    document.getElementById("news-title").querySelector("span").textContent =
      newsItem.title;
    document.getElementById("news-title-2").textContent = newsItem.title;
    document.getElementById("news-date").textContent = newsItem.date;
    // Önceki ve sonraki haber id'lerini al
    const newsIds = newsData.map((news) => news.id);
    console.log(newsIds);
    const currentNewsIndex = newsIds.indexOf(newsId);
    console.log(currentNewsIndex);

    // Eğer mevcut haber ilk haberse, önceki haber sonuncu haber olacak
    const prevNewsIndex =
      currentNewsIndex === 0 ? newsIds.length - 1 : currentNewsIndex - 1;

    // Eğer mevcut haber sonuncu haberse, sonraki haber ilk haber olacak
    const nextNewsIndex =
      currentNewsIndex === newsIds.length - 1 ? 0 : currentNewsIndex + 1;

    // Önceki haber
    const prevNewsId = newsIds[prevNewsIndex];
    const prevNewsUrl = `haber-detay.html?id=${prevNewsId}`;
    document
      .querySelector(".page-navigation__prev")
      .setAttribute("href", prevNewsUrl);

    // Sonraki haber
    const nextNewsId = newsIds[nextNewsIndex];
    const nextNewsUrl = `haber-detay.html?id=${nextNewsId}`;
    document
      .querySelector(".page-navigation__next")
      .setAttribute("href", nextNewsUrl);
    ...
    // Set innerHTML for the new WhatsApp link
...
`;
    ...
    // Paragrafları dinamik olarak ekle
  ...
  const existingBlockquote = document.getElementById("news-blockquote");
  const existingAuthor = document.getElementById("blockquote-author");
  ...
  if (newsItem.blockquote) {
    // Önce mevcut blockquote öğesini sil

    // Yeni blockquote öğesi oluştur
    const blockquote = document.createElement("blockquote");
    blockquote.id = "news-blockquote";

    // İçine text paragrafını ekle
    const textParagraph = document.createElement("p");
    textParagraph.id = "blockquote-text";
    textParagraph.textContent = newsItem.blockquote.text;
    blockquote.appendChild(textParagraph);

    // Yazar paragrafını ekle, yalnızca yazar mevcutsa
    if (newsItem.blockquote.author) {
      const authorParagraph = document.createElement("p");
      const cite = document.createElement("cite");
      cite.id = "blockquote-author";
      cite.textContent = newsItem.blockquote.author;
      authorParagraph.appendChild(cite);
      blockquote.appendChild(authorParagraph);
    }

    paragraphsContainer.appendChild(blockquote);
    if (newsItem.paragraphs2) {
      newsItem.paragraphs2.forEach((paragraph) => {
        const p = document.createElement("p");
        p.textContent = paragraph;
        paragraphsContainer.appendChild(p);
      });
  ...
  if (newsItem.photos1) {
    const photos1Container = document.createElement("div");
    photos1Container.classList.add("row", "gap-row");
    newsItem.photos1.forEach((photo) => {
      const photoDiv = document.createElement("div");
      photoDiv.classList.add("col-xs-12", "col-sm-12", "col-md-6", "col-lg-6");

      const anchor = document.createElement("a");
      anchor.href = photo;
      anchor.classList.add("mfp-image");
      ...
      photos1Container.appendChild(photoDiv);
    });
    paragraphsContainer.appendChild(photos1Container);
  }
});
